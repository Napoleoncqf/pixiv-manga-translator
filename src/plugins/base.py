...
class PluginBase:
    ...
    def before_rendering(self, image_to_render_on, translated_texts, bubble_coords, bubble_states, params):
        """
        在文本渲染之前调用。
        可以修改用于渲染的图像、文本、坐标或状态。

        Args:
            image_to_render_on (PIL.Image.Image): 将在其上渲染文本的基础图像。
            translated_texts (list): 要渲染的文本列表。
            bubble_coords (list): 气泡坐标。
            bubble_states (list): BubbleState 列表。
            params (dict): 处理参数。

        Returns:
            tuple(PIL.Image.Image, list, list, list) or None: 修改后的参数，或 None。
        """
