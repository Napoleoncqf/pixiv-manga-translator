...
def get_architecture_presets():
    """获取可用的分析架构预设"""
    try:
        return success_response(data={"presets": ARCHITECTURE_PRESETS})
...
def get_config():
    ...
    try:
        ...
        return success_response(data={"config": config.to_dict()})
    except Exception as e:
...
def save_config():
    ...
    try:
        ...
        if not data:
            return error_response("无效的配置数据", 400)
        ...
        return success_response(message="配置已保存" if success else "保存失败")
    except Exception as e:
...
def save_provider_config(config_type: str, provider: str):
    ...
    try:
        ...
        return success_response(message=f"{provider} 配置已保存" if success else "保存失败")
    except Exception as e:
...
def set_current(config_type: str):
    ...
    try:
        ...
        if not provider:
            return error_response("未指定服务商", 400)

        success = set_current_provider(config_type, provider)

        return success_response(message=f"已切换到 {provider}" if success else "切换失败")
    except Exception as e:
...
def test_vlm_connection():
    ...
    try:
        ...
        return success_response(message="连接成功" if success else "连接失败")
    except Exception as e:
...
def get_prompts_library():
    ...
    try:
        ...
        return success_response(data={"library": library})
    except Exception as e:
        logger.error(f"获取提示词库失败: {e}", exc_info=True)
        return error_response(str(e), 500, library=[])
...
def add_prompt_to_library():
    ...
    try:
        ...
        if not new_prompt or not new_prompt.get('id') or not new_prompt.get('name'):
            return error_response("提示词数据无效", 400)
        ...
        return success_response(message="提示词已添加到库")
    except Exception as e:
...
def delete_prompt_from_library(prompt_id):
    ...
    try:
        ...
        if not os.path.exists(library_path):
            return error_response("提示词库不存在", 404)
...
def import_prompts_library():
    ...
    try:
        ...
        return success_response(message=f"已导入 {len(library)} 个提示词")
    except Exception as e:
