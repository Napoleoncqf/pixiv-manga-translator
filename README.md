...
| 角色分类标签 | OCR 文本完全匹配 `長女/次女/三女` 等 | 保留原图不翻译不擦除 |
| Vision 补检 | 绕过 SFX/尺寸过滤，但标签词仍会过滤 | CTD 漏检的标题/简介 |
| 空译文 | 跳过擦除与渲染，保留原图 | Vision 看不清返回的空字符串 |

## 渲染方向判定

漫画中文约定：**默认竖排**（符合阅读习惯），只有明确应该横排的情况才横排。

| 条件 | 方向 |
|------|------|
| 译文含数字 | **横排**（避免小数点竖排错位） |
| 译文仅 1 字 | **横排** |
| 横扁标题框（宽 > 高 × 1.3） | **横排 + 大字号**（用于艺术字标题） |
| 气泡宽 > 高 × 1.5 | **横排** |
| 其他 | **竖排** |
...
- **艺术字/粗体装饰**：画面上的大号装饰字（如 `スマホ禁止！`）Vision 视觉识别准确率低，可能输出错误翻译
- **角色标签**：`長女/次女/三女` 等标签默认保留原图不翻译（汉字本身中文读者可读）
